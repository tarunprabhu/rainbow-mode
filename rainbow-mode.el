...
(defcustom rainbow-colorize-foreground nil
  "Whether to colorize the color string text instead of the background."
  :type 'symbol
  :options '(auto foreground-only)
  :group 'rainbow)
...
`rainbow-color-luminance', and is either white or black."
  (let* ((match (or match 0))
         (inverse (if (> 0.5 (rainbow-x-color-luminance color)) "white" "black"))
         (foreground (if rainbow-colorize-foreground color inverse))
         (background (pcase rainbow-colorize-foreground
                       ('auto inverse)
                       ('foreground-only 'unspecified)
                       (_ color))))
    (put-text-property
